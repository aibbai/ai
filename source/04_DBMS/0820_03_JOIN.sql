...
-- ▶ 1. EQIT JOIN : 공통필드 값이 일치되는 조건만 JOIN
...
-- 탄탄 EX. 뉴욕에서 근무하는 사원의 이름과 급여를 출력하시오
    SELECT ENAME, SAL FROM EMP E, DEPT D WHERE E.DEPTNO=D.DEPTNO AND LOC='NEW YORK';
    
-- 탄탄 EX. ACCOUNTING 부서 소속 사원의 이름과 입사일을 입사일 최신순으로 출력하시오
    SELECT ENAME, HIREDATE
        FROM EMP E, DEPT D
        WHERE E.DEPTNO=D.DEPTNO AND DNAME='ACCOUNTING'
        ORDER BY HIREDATE DESC;
        
-- 탄탄 EX. 직급이 MANAGER인 사원의 이름, 부서명을 출력하시오
    SELECT ENAME, DNAME
        FROM EMP E, DEPT D
        WHERE E.DEPTNO=D.DEPTNO AND JOB='MANAGER';

-- 탄탄 EX. Comm이 null이 아닌 사원의 이름, 급여, 부서코드, 근무지를 출력하시오
    SELECT ENAME, SAL, E.DEPTNO, LOC FROM EMP E, DEPT D WHERE E.DEPTNO=D.DEPTNO AND COMM IS NOT NULL;
    
-- EX. 20번부서 사원의 이름, 급여, 부서코드, 근무지를 부서코드순으로 출력하시오
...
-- 탄탄 EX1. Comm이 null이 아닌 사원의 이름, 급여, 등급, 부서번호, 부서이름, 근무지를 출력하시오.
...
-- 탄탄 EX2. 이름, 급여, 입사일, 급여등급
SELECT ENAME, SAL, HIREDATE, GRADE
    FROM EMP, SALGRADE WHERE SAL BETWEEN LOSAL AND HISAL;
    
-- 탄탄 EX3. 이름, 급여, 급여등급, 연봉, 부서명을 부서명순으로 정렬하여 출력. 부서가 같으면 연봉순. 연봉=(sal+comm)*12 comm이 null 면 0
SELECT ENAME, SAL, GRADE, (SAL+NVL(COMM, 0))*12 ANNUAL_SAL, DNAME
    FROM EMP E, DEPT D, SALGRADE
    WHERE E.DEPTNO=D.DEPTNO AND SAL BETWEEN LOSAL AND HISAL
    ORDER BY DNAME,(SAL+NVL(COMM, 0))*12;
    
-- 탄탄 EX4. 이름, 업무, 급여, 등급, 부서코드, 부서명 출력. 급여가 1000~3000사이. 정렬조건 : 부서별, 부서같으면 업무별, 업무같으면 급여 큰순
...
-- 탄탄 EX5. 이름, 급여, 등급, 입사일, 근무지. 81년에 입사한 사람. 등급 큰순
...
SELECT W.EMPNO, W.ENAME, W.MGR, M.EMPNO 상사사번, M.ENAME 상사이름
 ...
 -- 탄탄 EX. 매니저가 KING 인 사원들의 이름과 직급을 출력하시오
...
-- (1) SELF-JOIN에서의 OUTER JOIN : 배제된 행을 결과에 포함시킬 경우, +기호를 정보가 부족한 컬럼에 붙임
    SELECT W.ENAME, W.MGR, M.EMPNO, M.ENAME
        FROM EMP W, EMP M
        WHERE W.MGR = M.EMPNO(+);
        
    -- EX. 'SMITH'의 상사는 'FOAD'다 'KING'의 상사는 없다'
    SELECT W.ENAME || '의 상사는' || NVL(M.ENAME, '없') || '다'
        FROM EMP W, EMP M
        WHERE W.MGR = M.EMPNO(+);
        
    -- EX. 사원명, 상사사번, 상사사번, 상사명
    SELECT 사원.ENAME, 부하.MGR, 상사.EMPNO, 상사.ENAME
        FROM EMP 부하, EMP 상사
...
SELECT * FROM EMP E, DEPT D
    WHERE E.DEPNO=D.DEPTNO; -- 40번 부서 출력 안 됨
SELECT * FROM EMP E, DEPT D
...
--4. 상사가 없는 직원과 상사가 있는 직원 모두에 대해 이름, 급여, 부서코드, 부서명, 근무지, 직속상사명을 출력하시오(단, 직속상사가 없을 경우 직속상사명에는 ‘없음’으로 대신 출력하시오)
    SELECT W.ENAME, W.SAL, D.DEPTNO, D.DNAME, LOC, NVL(M.ENAME, '없음')
        FROM EMP W, EMP M, DEPT D
        WHERE W.MGR=M.EMPNO(+)
            AND W.DEPTNO=D.DEPTNO;
...
--7. 6번을 부서명 순으로 오름차순 정렬하여 출력(부서가 같으면 급여가 큰 순 정렬)
    SELECT W.ENAME, W.SAL, GRADE, D.DNAME, (W.SAL+NVL(W.COMM, 0))*12 ANNUAL_SAL, NVL(M.ENAME, '없음')
        FROM EMP W, EMP M, DEPT D, SALGRADE
        WHERE W.MGR=M.EMPNO(+)
            AND W.DEPTNO=D.DEPTNO
            AND W.SAL BETWEEN LOSAL AND HISAL
